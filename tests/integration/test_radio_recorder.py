...
def create_sample_recording():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        print(f"Recording 1-minute sample to {output_file}")
        ...
        if process.returncode == 0 and os.path.exists(output_file):
            # Upload to S3 in the recordings/samples folder
            s3_key = f"recordings/samples/recording_{timestamp}.mp3"
            s3_client.upload_file(output_file, S3_BUCKET, s3_key)
            print(f"Successfully uploaded sample to s3://{S3_BUCKET}/{s3_key}")
            return True
        else:
            print("Failed to create sample recording")
            return False
